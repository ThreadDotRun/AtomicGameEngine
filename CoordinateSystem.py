...
class CoordinateSystem:
    ...
    def get_version(self) -> str:
        try:
            with open(os.path.join(os.path.dirname(__file__), "CoordinateSystemVersion.md"), "r") as f:
                return f.read().strip()
        except:
    ...
    def get_info_message(self) -> str:
        try:
            with open(os.path.join(os.path.dirname(__file__), "CoordinateSystemInfo.md"), "r") as f:
                return f.read().strip()
        except:
            return "CoordinateSystem: A 3D coordinate management system."
    ...
    def update_entity_position(self, entity_id: EntityID, new_position: Coordinate3D) -> None:
        ...
        self._entity_positions[entity_id] = tuple(float(c) for c in new_position)
    ...
    def add_static_plane(self, geometry_id: GeometryID, origin: Coordinate3D, normal: Coordinate3D, category: str = "generic_plane") -> None:
        if geometry_id in self._static_geometry:
            raise ValueError(f"Static geometry with ID '{geometry_id}' already exists.")
        if not (isinstance(origin, tuple) and len(origin) == 3 and
                all(isinstance(coord, (int, float)) for coord in origin)):
            raise TypeError("Plane origin must be a tuple of three numbers (x, y, z).")
        if not (isinstance(normal, tuple) and len(normal) == 3 and
                all(isinstance(coord, (int, float)) for coord in normal)):
            raise TypeError("Plane normal must be a tuple of three numbers (nx, ny, nz).")
    ...
    def remove_static_geometry(self, geometry_id: GeometryID) -> None:
        if geometry_id not in self._static_geometry:
            raise KeyError(f"Static geometry with ID '{geometry_id}' not found.")
        del self._static_geometry[geometry_id]
